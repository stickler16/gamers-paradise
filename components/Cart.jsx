      ...
      <div className="cart-container">
            ...
            <div className="product" key={item._id}>
              ...
              <div className="item-desc">
                <div className="flex top">
                  <h5>{item.name}</h5>
                  <h4>Rs. {item.price.toLocaleString('en-IN')}</h4>
                </div>
        ...
        {cartItems.length >= 1 && (
          <div className="cart-bottom">
            <div className="total">
              <h3>Subtotal:</h3>
              <h3>Rs {totalPrice.toLocaleString('en-IN')}</h3>
            </div>
            <div className="btn-container">
              <button type="button" className="btn" onClick={handleCheckout}>Checkout (Stripe)</button>
            </div>
          </div>
        )}
      </div>
